...
use App\Models\AmericanTransaction;
...
class FetchAmericanBookTransactions extends Command
{
    protected $signature = 'fetch:amtransactions';
    protected $description = 'Fetch new transactions from API and store in database';

    public function handle()
    {
        $merchantAuthentication = new AnetAPI\MerchantAuthenticationType();
        $merchantAuthentication->setName(env('AMERICAN_LOG_KEY'));
        $merchantAuthentication->setTransactionKey(env('AMERICAN_LOG_SECRET'));

        // $firstSettlementDate = new DateTime("2025-06-01T06:00:00Z");
        // $lastSettlementDate = new DateTime();
        // $lastSettlementDate->setDate(2025, 06, 30);
        ...
        if ($response != null && $response->getMessages()->getResultCode() == "Ok") {
            ...
            Log::info('New American Transactions stored successfully');
        } else {
            Log::info('Failed to fetch american transactions');
        }
    }
    ...
    private function getTransactionsForBatch($merchantAuthentication, $batchId)
    {
        ...
        if ($response != null && $response->getMessages()->getResultCode() == "Ok") {
            foreach ($response->getTransactions() as $transaction) {
                ...
                $existingTransaction = AmericanTransaction::where('transaction_id', $transId)->first();
                ...
                AmericanTransaction::updateOrCreate(
